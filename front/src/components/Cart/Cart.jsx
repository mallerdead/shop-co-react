            ...
            {isLoading ? (
              ...
              cartItems.map((item, index) => (
                <Fragment key={item.id}>
                  <CartItem
                    cloth={item}
                    deleteHandler={() => deleteCartItem(item.id)}
            ...
            <div className={styles.subtotal}>
              ...
              <span>
                ...
                {isLoading
                  ? 0
                  : cartItems.reduce(
                      (accum, cur) => accum + (cur.discount ? +cur.oldPrice : +cur.price) * +cur.quantity,
                      0,
                    )}
              </span>
            </div>
            <div className={styles.discount}>
              ...
              <span>
                {isLoading
                  ? '$0'
                  : `-$${cartItems.reduce(
                      (accum, cur) => (cur.discount ? accum + (+cur.oldPrice - +cur.price) * cur.quantity : accum),
                      0,
                    )}`}
              </span>
            </div>
