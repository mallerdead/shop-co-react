...
export const Shop = () => {
  ...
  const applyFilters = () => {
    ...
    allClothes.forEach((cloth) => {
      if (
        (cloth.discount ? (cloth.price * (1 - cloth.discount / 100)).toFixed(2) : cloth.price) >= rangePrice[0] &&
        (cloth.discount ? (cloth.price * (1 - cloth.discount / 100)).toFixed(2) : cloth.price) <= rangePrice[1]
      ) {
        result.push(cloth)
      }
    })
    ...
    getClothes()
      .then((data) => {
        ...
        setRangePrice([
          Math.min.apply(
            null,
            data.map((cloth) => (cloth.discount ? (cloth.price * (1 - cloth.discount / 100)).toFixed(2) : cloth.price)),
          ), // TODO : Fix range with discounts
          Math.max.apply(
            null,
            data.map((cloth) => (cloth.discount ? (cloth.price * (1 - cloth.discount / 100)).toFixed(2) : cloth.price)),
          ),
