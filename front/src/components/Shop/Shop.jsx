...
export const Shop = () => {
    ...
    const setColorById = (id) => setFilterColors(prev => prev.map(
        filterColor => filterColor.id === id ? { ...filterColor, isActive: !filterColor.isActive } : filterColor)
    )

    const setSizeById = (id) => setFilterSizes(prev => prev.map(
        filterSize => filterSize.id === id ? { ...filterSize, isActive: !filterSize.isActive } : filterSize)
    )

    const setShowingFilter = (id) => setShowFilters(prev => prev.map(
        filter => filter.id === id ? { ...filter, isShowing: !filter.isShowing } : filter
    ))

    const applyFilters = () => {
        setFilterMenu(false)
        const result = []
        const filterSizesActive = filterSizes.filter(filterSize => filterSize.isActive).map(filterSize => filterSize.size)
        const filterColorsActive = filterColors.filter(filterColor => filterColor.isActive).map(filterColor => filterColor.color)
    ...
    useEffect(() => {
        setIsLoading(true)
        const response = getClothes()
        setAllClothes(response)
        setClothes(response)
        setFilterColors(findUnique(response.map(cloth => {
            return { id: cloth.color.id, color: cloth.color.colorName, isActive: false }
        })))
        setRangePrice([Math.min.apply(null, response.map(cloth => cloth.price)), Math.max.apply(null, response.map(cloth => cloth.price))])
        const sizes = []
        response.map(cloth => cloth.sizes.map(clothSize => {
            sizes.push({ id: clothSize.id, size: clothSize.size, isActive: false })
        }))
        setFilterSizes(findUnique(sizes))
            ...
            <div className={styles.shop}>
                <div className={`${styles.modal} ${filterMenu ? styles.active : ""}`}>
                    <div className={`${styles.filters} ${filterMenu ? styles.active : ""}`}>
                        ...
                        <span className={styles.delimiter}></span>
                        {<FilterItem title={showFilters[0].title} isShowing={showFilters[0].isShowing} onClickHandler={() => setShowingFilter(showFilters[0].id)} content={isLoading ? "" : <PriceSlider min={Math.min.apply(null, allClothes.map(cloth => cloth.price))} max={Math.max.apply(null, allClothes.map(cloth => cloth.price))} handleSliderChange={setRangePrice} />} />}
                        <span className={styles.delimiter}></span>
                        {<FilterItem title={showFilters[1].title} isShowing={showFilters[1].isShowing} onClickHandler={() => setShowingFilter(showFilters[1].id)} content={filterColors.map(filterColor => <Color key={filterColor.id} color={filterColor.color} isActive={filterColor.isActive} toggleActive={() => setColorById(filterColor.id)} />)} />}
                        <span className={styles.delimiter}></span>
                        {<FilterItem title={showFilters[2].title} isShowing={showFilters[2].isShowing} onClickHandler={() => setShowingFilter(showFilters[2].id)} content={isLoading ? "" : filterSizes.map(filterSize => <Size key={filterSize.id} size={filterSize.size} isActive={filterSize.isActive} toggleActive={() => setSizeById(filterSize.id)} />)} />}
                        <span className={styles.delimiter}></span>
                        {<FilterItem title={showFilters[3].title} isShowing={showFilters[3].isShowing} onClickHandler={() => setShowingFilter(showFilters[3].id)} content={<div className={styles.styles}><button>Casual</button><button>Formal</button><button>Party</button><button>Gym</button></div>} />}
                        <button className={styles.applyFilter} onClick={applyFilters}>Apply Filter</button>
                    </div>
                </div>
                <div className={styles.clothesWrapper}>
                    <div className={styles.clothesTitle}>
                        ...
                        {isLoading ?
                            ...
                            <div className={styles.sortClothes}>
                                <span className={styles.sort}>Showing {(currentPage - 1) * clothesPerPage} - {clothesPerPage * currentPage > clothes.length ? clothes.length : clothesPerPage * currentPage} of {clothes.length} Products</span>
                    ...
                    <div className={styles.clothes}>
                        {isLoading ? <LoadingSpinner /> : clothes.slice((currentPage - 1) * clothesPerPage, clothesPerPage * currentPage).map(cloth => <ClothItem key={cloth.id} cloth={cloth} />)}
                    </div>
                    {isLoading ? "" : <Pagination activePage={currentPage} countPerPage={clothesPerPage} arrayLength={clothes.length} setCurrentPage={setCurrentPage} />}
                </div>
            </div>
        </div >)
}