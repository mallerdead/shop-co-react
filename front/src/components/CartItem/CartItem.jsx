...
export const CartItem = ({ cloth, deleteHandler, changeQuantity }) => {
    const [count, setCount] = useState(cloth.quantity)
    ...
    return (
        <div className={styles.item}>
            <div className={styles.clothPreview}>
                <img src={`/src/assets/${cloth.imageLink}`} alt="" />
            </div>
            <div className={styles.description}>
                ...
                <div className={styles.wrapper}>
                    ...
                    <div className={styles.size}>Size: <span>{cloth.size}</span></div>
                    <div className={styles.color}>Color: <span>{cloth.color}</span></div>
                </div>
                <div className={styles.total}>
                    <div className={styles.price}>${cloth.price}</div>
                    <div className={styles.count}>
                        <button className={styles.decrement} onClick={() => {
                        ...
                        <input type="text" value={count} onChange={e => setCount(e.target.value)} ref={countInput}
                            onBlur={() =>
                                setCount(prev => {
                                    const onlyNumbers = prev.toString().replace(/[^0-9]/g, '')
                                    return onlyNumbers ? onlyNumbers : 1
                                })} />
                        <button className={styles.increment} onClick={() => {
                            countInput.current.focus()
                            setCount(prev => ++prev)
                        }}>
